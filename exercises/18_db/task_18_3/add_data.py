...
query_write_into_dhcp_snooping_table = '''insert into dhcp (mac, ip, vlan, interface, switch, active) values (?, ?, ?, ?, ?, 1)'''
        ...
        for row in list_of_tuples:
            try:
                ...
                if len(row) > 2:
                    ...
                    conn.execute("update dhcp set ip = '"+row[1]+"', vlan = "+row[2]+", interface = '"+row[3]+"', switch ='"+row[4]+"', active = 1 where mac = '"+row[0]+"'")
                    conn.commit()         
    else:
        try:
            print('set ACTIVE = 0')
